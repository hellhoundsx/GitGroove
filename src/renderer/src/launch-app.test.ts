...
// This test covers `tools/launch-app.mjs`, not the renderer. It lives here for the same reason
// `repo-hygiene.test.ts` does: `vitest.config.ts` includes `src/**/*.test.ts` and
// `tsconfig.web.json` includes `src/renderer/src/**/*`, so `npm test` and `npm run typecheck`
// both pick it up with no configuration change. Move it and both of those need editing.
//
